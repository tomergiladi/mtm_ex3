 ...
 *   				  set, and returdsadns it.
 *   setGetNext		- Advances the internal iterator to the next element and
 *   				  returns it.
 *   setAdd			- Adds a new element to the set.
 *   setRemove		- Removes an element which matches a given element (by
 *the compare function). Resets the internal iterator.
 *	 setClear		- Clears the contents of the set. Frees all the
 *elements of the set using the free function. SET_FOREACH	- A macro for
 *iterating over the set's elements.
 */
 ...
 * 		inside the set. Used to check if new elements already exist in
 * the set.
 * @return
 ...
 *	lowest value as determined by the comparison function used to initialize
 *the set. Use this to start iteraing over the set. Use (To continue iteration
 *use setGetNext)
 *
 ...
 * @return
 * 	NULL if reached the end of the set, or the iterator is at an invalid
 *state or a NULL sent as argument The next element on the set in case of
 *success
 */
 ...
 * 	SET_OUT_OF_MEMORY if an allocation failed (Meaning the function for
 *copying an element failed) SET_ITEM_ALREADY_EXISTS if an equal item already
 *exists in the set SET_SUCCESS the element has been inserted successfully
 */
SetResult setAdd(Set set, SetElement element);

/**
 * 	setRemove: Removes an element from the set. The element is found using
 * the comparison function given at initialization. Once found, the element is
 * 	removed and deallocated using the free function supplied at
 * initialzation. Iterator's value is undefined after this operation.
 *
 * @param set -
