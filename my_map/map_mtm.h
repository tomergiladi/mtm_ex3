 ...
 *(key,data) elements for which the key matches a given element (by the key
 *compare function). This resets the internal iterator. mapGetFirst	- Sets
 *the internal iterator to the first key in the map, and returns it. mapGetNext
 *- Advances the internal iterator to the next key and returns it. mapClear
 *- Clears the contents of the map. Frees all the elements of the map using the
 *free function. MAP_FOREACH	- A macro for iterating over the map's elements.
 */
 ...
 * elements inside the map. Used to check if new elements already exist in the
 * map.
 * @return
 ...
 *function which is given at initialization and old data memory would be deleted
 ...
 * @param keyElement
 * 	The key element to find and remove from the map. The element will be
 * freed using the free function given at initialization. The data element
 * associated with this key will also be freed using the free function given at
 * initialization.
 * @return
 ...
 *this to start iteraing over the map. To continue iteration use mapGetNext
 *
...
#define MAP_FOREACH(type, iterator, map)                   \
    for (type iterator = (type)mapGetFirst(map); iterator; \
         iterator = mapGetNext(map))

#endif /* MAP_MTM_H_ eqwew*/
